...
export function convertToBPMN(nodes: AppNode[], edges: Edge[]): string {
    const processId = "SnapFlow_Process";
    ...
    nodes.forEach(node => {
        ...
        switch (node.type) {
            ...
            case 'end':
                ...
                break;
            case 'task':
                const assignee = (node.data as any).config?.assignee || 'kermit';
                xml += `        <userTask id="${id}" name="${name}" flowable:assignee="${assignee}" />\n`;
                break;
            case 'aiAgent':

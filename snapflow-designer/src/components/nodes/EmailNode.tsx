...
export function EmailNode({ data, selected }: NodeProps<AppNode>) {
    return (
        <div className={`px-4 py-3 shadow-md rounded-md bg-white border-2 transition-all duration-200 ${selected ? 'border-gray-900 ring-2 ring-gray-300' : 'border-gray-400 hover:border-gray-600'}`}>
            <div className="flex items-center gap-3">
                ...
                <div>
                    <div className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Email Task</div>
                    <div className="text-sm font-bold text-gray-900">{data.label}</div>
                    {data.config?.recipient && (
                        <div className="text-[10px] text-gray-500 mt-0.5">To: {data.config.recipient}</div>
                    )}
                </div>
            </div>
